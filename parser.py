...
def __normalize_initializer_list(value, node, treesitter_parser, cpp_language, source_code):
    query = cpp_language.query(INITIALIZER_LIST_QUERY)
    tree = treesitter_parser.parse(source_code)

    captures = query.captures(tree.root_node, start_point=node.start_point, end_point=node.end_point)
    ...
    for c in captures:
        ...
        if capture_label == 'name':
            ...
            fields[current_field] = __normalize_param(param, list_node, treesitter_parser, cpp_language, source_code)['value'] if current_field else node.text.decode('utf-8')
            current_field = None
...
def __normalize_param(param, node, treesitter_parser, cpp_language, source_code):
    if param['type'] == 'string_literal' or param['type'] == 'raw_string_literal':
        ...
        param['value'] = __normalize_initializer_list(param['value'], node, treesitter_parser, cpp_language, source_code)
    else:
...
def extract_properties_from_file_pair(treesitter_parser, cpp_language, file_pair: FilePair):
    header_properties = parse_cpp_header(treesitter_parser, cpp_language, get_file_contents(file_pair.header))

    if len(header_properties) == 0:
        return PropertyBag()

    source_properties =  parse_cpp_source(treesitter_parser, cpp_language, get_file_contents(file_pair.implementation))
