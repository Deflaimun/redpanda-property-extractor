...
class BasicInfoTransformer:
    ...
    def parse(self, property, info, file_pair):
        ...
        property['defined_in'] = re.sub(r'^.*src/', 'src/', str(file_pair.implementation))
        property['description'] = info['params'][1]['value'] if len(info['params']) > 1 else None
...
class IsNullableTransformer:
    ...
    def parse(self, property, info, file_pair):
        if len(info['params']) > 2 and 'required' in info['params'][2]['value']:
            is_required = re.sub(r'^.*::', '', info['params'][2]['value']['required']) == 'yes'
...
class IsArrayTransformer:
    ...
    def parse(self, property, info, file_pair):
        ...
        property['items']['type'] = self.type_transformer.get_type_from_declaration(info['declaration'])

class NeedsRestartTransformer:
    def accepts(self, info, file_pair):
        return True if len(info['params']) > 2 and 'needs_restart' in info['params'][2]['value'] else False

    def parse(self, property, info, file_pair):
        needs_restart = re.sub(r'^.*::', '', info['params'][2]['value']['needs_restart'])
        property['needs_restart'] = needs_restart == 'yes'

class VisibilityTransformer:
    def accepts(self, info, file_pair):
        return True if len(info['params']) > 2 and 'visibility' in info['params'][2]['value'] else False

    def parse(self, property, info, file_pair):
        property['visibility'] = re.sub(r'^.*::', '', info['params'][2]['value']['visibility'])
...
class TypeTransformer:
    ...
    def get_cpp_type_from_declaration(self, declaration):
        ...
        raw_type = re.sub(r'^.*property<(.+)>.*', '\\1', one_line_declaration).split()[0].replace(',', '')
...
class DeprecatedTransformer:
    def accepts(self, info, file_pair):
        return 'deprecated_property' in info['declaration'] \
            or (len(info['params']) > 2 and 'visibility' in info['params'][2]['value'] and 'deprecated' in info['params'][2]['value']['visibility'])
...
class IsSecretTransformer:
    def accepts(self, info, file_pair):
        return True if len(info['params']) > 2 and 'secret' in info['params'][2]['value'] else False
