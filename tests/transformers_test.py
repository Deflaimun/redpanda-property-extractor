...
def create_property_info(name, description, declaration = None, metadata = None, default_value = '{}', default_value_type = 'initializer_list'):
    info = PropertyBag()
...
class IsArrayTransformerTestCase(unittest.TestCase):
    ...
    def test_accepts_vector(self):
        self.assertTrue(self.transformer.accepts(create_property_info("test_property", "test description", "std::vector<int>"), None))

    def test_rejects_other_properties(self):
        for t in ["int", "std::optional", "config::endpoint_tls_config", "deprecated_property"]:
            with self.subTest(t):
                self.assertFalse(self.transformer.accepts(create_property_info("test_property", "test description", t), None))

    def test_adds_array_field(self):
        property = PropertyBag()
        info = create_property_info("test_property", "test description", "std::vector<int>")
        self.transformer.parse(property, info, FilePair('testfile.h', 'testfile.cc'))
...
class NeedsRestartTransformerTestCase(unittest.TestCase):
    ...
    def test_accepts_needs_restart_metadata(self):
        self.assertTrue(self.transformer.accepts(create_property_info("test_property", "test description", "bool", PropertyBag(needs_restart='needs_restart::no')), None))
        self.assertTrue(self.transformer.accepts(create_property_info("test_property", "test description", "bool", PropertyBag(needs_restart='needs_restart::yes')), None))

    def test_rejects_without_needs_restart(self):
        self.assertFalse(self.transformer.accepts(create_property_info("test_property", "test description", "bool", PropertyBag()), None))

    def test_adds_needs_restart_true(self):
        property = PropertyBag()
        info = create_property_info("test_property", "test description", "std::vector<int>", PropertyBag(needs_restart='needs_restart::yes'))
        self.transformer.parse(property, info, FilePair('testfile.h', 'testfile.cc'))
    ...
    def test_adds_needs_restart_false(self):
        property = PropertyBag()
        info = create_property_info("test_property", "test description", "std::vector<int>", PropertyBag(needs_restart='needs_restart::no'))
        self.transformer.parse(property, info, FilePair('testfile.h', 'testfile.cc'))
...
class VisibilityTransformerTestCase(unittest.TestCase):
    ...
    def test_accepts_visibility_metadata(self):
        self.assertTrue(self.transformer.accepts(create_property_info("test_property", "test description", "bool", PropertyBag(visibility='visibility::user')), None))

    def test_rejects_without_visibility(self):
        self.assertFalse(self.transformer.accepts(create_property_info("test_property", "test description", "bool", PropertyBag()), None))

    def test_adds_visibility(self):
        for p in [("user", "visibility::user"), ("tunable", "visibility::tunable"), ("deprecated", "visibility::deprecated")]:
            with self.subTest(p[0]):
                property = PropertyBag()
                info = create_property_info("test_property", "test description", "std::vector<int>", PropertyBag(visibility=p[1]))
                self.transformer.parse(property, info, FilePair('testfile.h', 'testfile.cc'))
...
class TypeTransformerTestCase(unittest.TestCase):
    ...
    def test_parse_simple_types(self):
        ...
        for t in types:
            with self.subTest(t[0]):
                property = PropertyBag()
                info = create_property_info("test_property", "test description", 'property<' + t[0] + '>' + ' test_property;')
                self.transformer.parse(property, info, FilePair('testfile.h', 'testfile.cc'))
    ...
    def test_parse_bounded_property(self):
        property = PropertyBag()
        info = create_property_info("test_property", "test description", 'bounded_property<double, numeric_bounds> disk_reservation_percent;')
        self.transformer.parse(property, info, FilePair('testfile.h', 'testfile.cc'))
    ...
    def test_parse_deprecated_property(self):
        property = PropertyBag()
        info = create_property_info("test_property", "test description", 'deprecated_property test_property;')
        self.transformer.parse(property, info, FilePair('testfile.h', 'testfile.cc'))
...
class DeprecatedTransformerTestCase(unittest.TestCase):
    ...
    def test_accepts_deprecated_visibility(self):
        self.assertTrue(self.transformer.accepts(create_property_info("test_property", "test description", "bool", PropertyBag(visibility='visibility::deprecated')), None))

    def test_accepts_deprecated_property_type(self):
        self.assertTrue(self.transformer.accepts(create_property_info("test_property", "test description", "deprecated_property", PropertyBag()), None))
    ...
    def test_adds_deprecated_field_from_deprecated_visibility(self):
        property = PropertyBag()
        info = create_property_info("test_property", "test description", "std::vector<int>", PropertyBag(visibility='visibility::deprecated'))
        self.transformer.parse(property, info, FilePair('testfile.h', 'testfile.cc'))
    ...
    def test_adds_deprecated_field_from_deprecated_property_type(self):
        property = PropertyBag()
        info = create_property_info("test_property", "test description", "deprecated_property", PropertyBag())
        self.transformer.parse(property, info, FilePair('testfile.h', 'testfile.cc'))
...
class IsSecretTransformerTestCase(unittest.TestCase):
    ...
    def test_accepts_with_secret(self):
        self.assertTrue(self.transformer.accepts(create_property_info("test_property", "test description", "bool", PropertyBag(secret='is_secret::yes')), None))
        self.assertTrue(self.transformer.accepts(create_property_info("test_property", "test description", "bool", PropertyBag(secret='is_secret::no')), None))
    ...
    def test_adds_is_secret_true(self):
        property = PropertyBag()
        info = create_property_info("test_property", "test description", "std::vector<int>", PropertyBag(secret='is_secret::yes'))
        self.transformer.parse(property, info, FilePair('testfile.h', 'testfile.cc'))
        self.assertTrue(property['is_secret'])

    def test_adds_is_secret_false(self):
        property = PropertyBag()
        info = create_property_info("test_property", "test description", "std::vector<int>", PropertyBag(secret='is_secret::no'))
        self.transformer.parse(property, info, FilePair('testfile.h', 'testfile.cc'))
